...
const Blog: React.FC<BlogProps> = ({ language, onBack }) => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/blog/posts`); // Goi API tuong doi
      ...
      >
        ...
        {!isLoading && !error && posts.length > 0 && postItemFullWidth > 0 && (
          <motion.div
            ...
            animate={animationControls as AnimationControls & {x: number | string}} // type assertion
            style={listStyle as React.CSSProperties} // type assertion
          >
